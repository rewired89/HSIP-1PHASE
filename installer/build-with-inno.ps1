...
if (-not $SkipBuild) {
    ...
    Write-Host "  Building hsip-cli.exe with full features..." -ForegroundColor Gray
    ...
    Write-Host "  Building hsip-gateway.exe..." -ForegroundColor Gray
...
$gatewayExe = "target\release\hsip-gateway.exe"
...
if (-not (Test-Path $gatewayExe)) {
    Write-Host "ERROR: $gatewayExe not found!" -ForegroundColor Red
    Write-Host "Run without -SkipBuild to build executables" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "  ✓ hsip-gateway.exe: $((Get-Item $gatewayExe).Length / 1MB | ForEach-Object {[math]::Round($_, 2)}) MB" -ForegroundColor Green
Write-Host ""
