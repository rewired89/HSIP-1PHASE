...
enum Commands {
    ...
    AuditExport {
        ...
        #[arg(long, default_value = "audit_log_export.json")]
        out: String,
        /// PostgreSQL connection string (if using --features postgres)
        #[arg(long)]
        db: Option<String>,
        /// Limit number of entries (0 = all)
        #[arg(long, default_value_t = 0)]
        limit: usize,
    },
    ...
    #[command(name = "audit-query")]
    AuditQuery {
        /// Filter by destination domain
        #[arg(long)]
        destination: Option<String>,
        /// Filter by decision type (Allow, Block, Quarantine)
        #[arg(long)]
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::AuditExport { out, db, limit } => {
            use hsip_telemetry_guard::PostgresAuditLog;

            let conn_str = db.unwrap_or_else(|| {
                std::env::var("DATABASE_URL")
                    .unwrap_or_else(|_| "postgresql://localhost/hsip_audit".to_string())
            });
            ...
            rt.block_on(async {
                let audit_log = PostgresAuditLog::new(conn_str);

                match audit_log.init().await {
                    Ok(()) => {
                        println!("[AUDIT] Connected to PostgreSQL audit database");
                        ...
                        match json {
                            Ok(json_str) => {
                                fs::write(&out, json_str).expect("Failed to write file");
                                println!("[AUDIT] Exported audit log to {}", out);
            ...
            rt.block_on(async {
                ...
                match audit_log.init().await {
                    Ok(()) => {
                        ...
                        match audit_log.verify_chain().await {
                            Ok(true) => {
                            ...
                            Ok(false) => {
                                ...
                                eprintln!("[AUDIT] Audit log may have been tampered with!");
                                std::process::exit(1);
                            }
                            Err(e) => eprintln!("[AUDIT] Verification failed: {}", e),
                        }
                    }
                    Err(e) => eprintln!("[AUDIT] Failed to connect to database: {}", e),
                }
                ...
                match audit_log.init().await {
                    Ok(()) => {
                        ...
                        if filtered.is_empty() {
                            println!("[AUDIT] No matching entries found");
                        } else {
        ...
        Commands::AuditStats { db } => {
            ...
            rt.block_on(async {
                ...
                match audit_log.init().await {
                    Ok(()) => {
                        let count = audit_log.len().await.unwrap_or(0);
                        let verified = audit_log.verify_chain().await.unwrap_or(false);
