...
pub mod http {
    ...
    pub async fn serve(addr: SocketAddr) -> anyhow::Result<()> {
        ...
        let app = Router::new()
            ...
            .route("/.well-known/hsip-public-key.txt", get(get_public_key))
            .with_state(state);
    ...
    /// Serve public key for HSIP identity verification
    /// RFC 8615 compliant .well-known endpoint
    async fn get_public_key() -> impl IntoResponse {
        use std::path::PathBuf;

        // Try to load public key from keystore
        ...
        match fs::read_to_string(&key_path) {
            ...
            Err(_) => {
                // If no key exists, return a placeholder explaining setup
                let placeholder = r#"# HSIP Public Key
#
# No public key found. To publish your public key:
#
# 1. Generate an Ed25519 keypair:
#    hsip-cli keygen
#
# 2. The public key will be saved to ~/.hsip/identity.pub
#
# 3. This endpoint will automatically serve your public key
#
# For more information: https://hsip.io/docs/identity
"#;
                (
                    axum::http::StatusCode::OK,
                    [("Content-Type", "text/plain; charset=utf-8")],
