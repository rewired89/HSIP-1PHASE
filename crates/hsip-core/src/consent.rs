...
/// Policy/reputation flags for consent request evaluation
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ConsentRequestFlags {
    /// Request is from previously unknown peer
    pub unknown_peer: bool,
    /// Peer was denied consent before
    pub denied_before: bool,
    /// Number of failed attempts from this peer
    pub failed_attempts: u32,
    /// Request triggered rate limiting
    pub rate_limited: bool,
    /// Request has invalid or suspicious fields
    pub suspicious: bool,
}

impl Default for ConsentRequestFlags {
    fn default() -> Self {
        Self {
            unknown_peer: true,
            denied_before: false,
...
/// Extended metadata for consent request evaluation and display
/// This includes cryptographically verified data (peer ID, timestamp)
/// and request-provided fields (purpose).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ConsentRequestMetadata {
    /// Cryptographically verified peer ID (derived from signature)
    pub peer_id: String,
    /// Request purpose string (from request, not verified)
    pub purpose: String,
    /// Request timestamp (from request, signature-verified)
    pub timestamp_ms: u64,
    /// Policy evaluation flags
    pub flags: ConsentRequestFlags,
}
