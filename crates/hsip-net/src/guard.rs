...
pub struct GuardCfg {
    ...
    /// Max number of consent requests allowed per minute per IP.
    pub max_consent_req_per_min: u32,
    /// Maximum accepted control frame length (post-prefix).
    pub max_frame_len: usize,
    /// Maximum accepted HELLO message length.
    pub max_hello_len: usize,
    /// Maximum accepted consent request length.
    pub max_consent_request_len: usize,
    /// Maximum accepted consent response length.
    pub max_consent_response_len: usize,
...
impl Default for GuardCfg {
    fn default() -> Self {
        ...
        Self {
            ...
            max_consent_req_per_min: 30,
            max_frame_len: MAX_CONTROL_FRAME_SIZE,
            max_hello_len: MAX_HELLO_SIZE,
            max_consent_request_len: MAX_CONSENT_REQUEST_SIZE,
            max_consent_response_len: MAX_CONSENT_RESPONSE_SIZE,
            pad_to_sizes: vec![],
        }
    }
}
...
pub struct Guard {
    ...
    consent_req_per_min: HashMap<IpAddr, WindowCounter>,
...
impl Guard {
    ...
    pub fn new(cfg: GuardCfg) -> Self {
        Self {
            ...
            consent_req_per_min: HashMap::new(),
            pinned: HashSet::new(),
    ...
    pub fn debug_banner(&self) {
        eprintln!(
            "[GuardDebug] enable={} pin={}min e1/5s={} badsig/min={} ctrl/min={} consent_req/min={} max_frame={} max_hello={} max_consent_req={} max_consent_resp={} pad={:?}",
            self.cfg.enable,
            self.cfg.pin_minutes,
            self.cfg.max_e1_per_5s,
            self.cfg.max_bad_sig_per_min,
            self.cfg.max_ctrl_per_min,
            self.cfg.max_consent_req_per_min,
            self.cfg.max_frame_len,
            self.cfg.max_hello_len,
            self.cfg.max_consent_request_len,
            self.cfg.max_consent_response_len,
    ...
    /// Count a consent request from `ip` against the per-minute window.
    ///
    /// # Errors
    /// Returns a rate-limit error string when exceeded.
    pub fn on_consent_request(&mut self, ip: IpAddr, len: usize) -> Result<(), String> {
        if !self.cfg.enable {
            return Ok(());
        }
        ...
        if self.is_blocklisted(ip) {
            self.mark_blocked(ip);
            eprintln!("[Guard] consent request from {ip} blocked by tracker wall");
            return Err("blocked by tracker wall".into());
        }

        if len > self.cfg.max_consent_request_len {
            self.mark_blocked(ip);
            return Err(format!(
                "consent request too large: {} > {}",
                len, self.cfg.max_consent_request_len
            ));
        }

        let entry = self.consent_req_per_min.entry(ip).or_insert_with(|| {
            WindowCounter::new_per(Duration::from_secs(60), self.cfg.max_consent_req_per_min)
        });
        match entry.hit() {
            Ok(()) => Ok(()),
            Err(e) => {
    ...
    /// Validate consent response size (does not rate limit, as responses are inbound from recipients).
    ///
    /// # Errors
    /// Returns an error if the response exceeds the configured maximum.
    pub fn validate_consent_response_size(&mut self, ip: IpAddr, len: usize) -> Result<(), String> {
        if !self.cfg.enable {
            return Ok(());
        }

        if len > self.cfg.max_consent_response_len {
            self.mark_blocked(ip);
            Err(format!(
                "consent response too large: {} > {}",
                len, self.cfg.max_consent_response_len
            ))
        } else {
            Ok(())
        }
    ...
    /// Validate HELLO message size before expensive signature verification.
    ///
    /// # Errors
    /// Returns an error if the HELLO exceeds the configured maximum.
    pub fn validate_hello_size(&mut self, ip: IpAddr, len: usize) -> Result<(), String> {
        if !self.cfg.enable {
            return Ok(());
        }

        if len > self.cfg.max_hello_len {
            self.mark_blocked(ip);
            Err(format!(
                "HELLO message too large: {} > {}",
                len, self.cfg.max_hello_len
            ))
        } else {
            Ok(())
        }
