...
HSIP is a UDP protocol with mandatory ChaCha20-Poly1305 encryption.
SSL stripping attacks don't apply - HSIP doesn't use HTTP/TLS.

For proper HSIP encryption testing, use:
    PowerShell: .\hsip_encryption_test.ps1
    or manually capture UDP traffic: tcpdump -i any -w capture.pcap 'udp port 50505'
                then verify no plaintext: strings capture.pcap
