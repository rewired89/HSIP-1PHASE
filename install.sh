...
main() {
  ...
  printf "  ${BOLD}hsip --help${RESET} — server startup options\n"
  say ""
  say "This is the hsip-api server binary. For the separate hsip-cli tool"
  say "(agent management, trust, key rotation), build it from source:"
  say "  cargo build --release -p hsip-cli"
  say ""
  say "Your API key will be saved to: ~/.hsip/admin.key"
  say "Docs and API reference: http://127.0.0.1:7474/docs  (once running)"
  say ""
