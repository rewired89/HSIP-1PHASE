...
HSIP (High Security Internet Protocol) enables peer-to-peer encrypted communication that requires **mutual cryptographic consent** before any data exchange. Communication only happens when both parties explicitly agree - enforced at the protocol level through capability tokens and signed handshakes.
...
### Security Hardening (v0.1.2+)

| Attack | HSIP Defense |
|--------|--------------|
| **DoS / Flooding** | Token bucket rate limiting (configurable limits per IP) |
| **Resource exhaustion** | Connection guards with global/per-IP limits |
| **Slowloris attacks** | Handshake, idle, and I/O timeouts |
| **Injection attacks** | Input validation and sanitization |
| **Timing attacks** | Constant-time cryptographic operations |
| **Memory dumps** | Automatic secret zeroization on drop |
| **Side-channel leaks** | Constant-time comparisons prevent timing leaks |
| **Weak ciphers** | TLS 1.3 wrapper with strong cipher suite enforcement |

See [SECURITY_HARDENING.md](SECURITY_HARDENING.md) for detailed security features.
