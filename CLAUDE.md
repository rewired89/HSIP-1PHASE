...
- **Database-compromise protection** — per-transaction key derivation (§4.29 above) explicitly does not protect against an attacker who already has read access to this database, since `tenant_id` still links every record there for ordinary multi-tenant operation. Raised as the direct follow-up question after §4.29 landed; not yet designed or built.
