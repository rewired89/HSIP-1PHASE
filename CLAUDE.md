...
- **Live-verify the `windows`-crate shortcut replacement on a real Windows machine** — §4.32/§4.33 replaced `mslnk` with direct `IShellLinkW`/`IPersistFile` COM calls (and fixed a COM apartment-mode edge case in §4.33), both cross-checked against the real crate source and confirmed to cross-compile and link cleanly for `x86_64-pc-windows-gnu`, but this sandbox has no Windows host to actually run `maybe_self_install`/`create_shortcuts` and confirm a working `.lnk` appears on a real Desktop/Start Menu. Stays open until a real Windows user's build (the same "found by a real user" pattern every other Windows-only gotcha in this document relies on) confirms it end-to-end.
