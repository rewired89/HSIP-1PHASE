...
- `GET /v1/decisions` now reports which named connection recorded each decision (`agent_key_id`/`agent_name`, via a `LEFT JOIN` on `api_keys` — a since-revoked key still lists its past decisions, just with `agent_name: null`) and accepts `?agent_key_id=`/`?since_ms=`/`?until_ms=` filters, expressed as `($N IS NULL OR col = $N)` in one fixed query so both `sqlx::Any` backends stay covered without dynamic SQL. Closes a real UX gap raised directly by using the product: the "AI Decisions" dashboard page could show a plain-language decision feed but couldn't say *which* connected agent a decision belonged to, or narrow a lookup to one agent/time window — exactly what a compliance/audit reviewer needs first ("what did this agent do, in this window"), not a raw signature bundle. `DecisionsSimple.jsx`'s `DecisionRow` now shows a `🤖 <agent name>` badge per row, and the page gained agent/time-window filter dropdowns (backed by the existing `GET /v1/agents`) that re-query on change. Covered by `test_decisions_list_reports_agent_name_and_filters_by_agent_and_time`. Also fixed a real, unrelated bug found in the same pass: `AIWatch.jsx` had `BASE_URL` hardcoded to `http://127.0.0.1:7777` (the wrong port for every deployment mode this project documents), silently breaking every copy-paste setup snippet (Siri Shortcut, Claude Desktop, capabilities URL) on that page — now `window.location.origin`, matching the pattern `DecisionsSimple.jsx` already used correctly.
