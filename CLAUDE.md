...
### Why HSIP has no built-in AWS/cloud dependency — this is deliberate, not a limitation

HSIP runs entirely on the machine it's installed on — desktop mode by default, server mode still self-hosted, no managed cloud service anywhere in its own critical path (no cloud KMS, no managed DB by default, no SaaS auth provider). This was a deliberate target-market decision, not just "hasn't gotten to cloud support yet":

**The target industries this serves are ones already heavily dependent on AWS (or another single cloud vendor) for their actual business** — trading systems, fintech, anything running its core infrastructure on one provider. For exactly that audience, an identity/consent/audit layer that *also* runs on AWS shares fate with it: an AWS outage, a misconfigured IAM policy, a compromised AWS account, or even just an AWS-side billing/ToS dispute would take down the same system meant to provide independent, tamper-evident proof of what happened. A trust and audit layer that depends on the same infrastructure it's supposed to be independently attesting about is a weaker guarantee than one that doesn't. Running HSIP locally means:
- **No shared blast radius.** An AWS-wide incident (or an incident specific to *that customer's* AWS account) doesn't take HSIP down with it — HSIP keeps signing, verifying, and recording locally regardless of what's happening to the cloud infrastructure it's providing accountability over.
- **No second vendor with access to the data.** The whole point of `accountable_key`/decision attestations/consent records is proving what happened without having to trust HSIP's own word for it (see Decision Attestations, Receipt Collection below) — running it on someone else's cloud reintroduces exactly the "trust a third party" dependency the design is trying to remove.
- **No new subpoena/compliance surface.** Data residency, cross-border transfer rules, and law-enforcement request handling are all simpler when nothing ever leaves the operator's own machine by default.
- **No recurring cloud cost or new operational dependency** for what's meant to be a lightweight, always-available local service.

This is the same reasoning that shaped Receipt Collection (below): even when a business needs *some* centralized visibility across many people's/agents' usage, the answer is "collect only small, already-signed proofs centrally," not "move everyone's operational data to a shared cloud database." Mutual TLS and PostgreSQL support exist for operators who *choose* to run HSIP as a shared, network-reachable service anyway — that's opt-in server mode, never the default, and still self-hosted rather than tied to a specific cloud vendor's managed services.

**What this doesn't mean:** HSIP isn't anti-cloud in general, and nothing stops an operator from running the `hsip-api` binary *on* an EC2 instance or similar if they want to — the point is that HSIP itself has no built-in, load-bearing dependency on any specific cloud vendor's services (KMS, managed Postgres, IAM, etc.), so its guarantees don't degrade if that vendor has a bad day.
...
| `hsip-dns` | — | UDP DNS server (:5300). Hardcoded tracker blocklist; forwards to 1.1.1.1:53 over a `connect()`-ed socket with transaction-ID validation (response spoofing fix, see below) — no longer accepts a forwarded reply from an arbitrary source. | Working |
...
POST/GET  /v1/decisions             Sign + chain an AI-agent decision attestation / list this tenant's decisions (GET supports `?agent_key_id=`, `?since_ms=`, `?until_ms=` filters; response rows include `agent_key_id`/`agent_name`)
...
POST      /v1/receipts/submit       Submit a proof bundle from another HSIP instance — verified before storing, see Receipt Collection below
GET       /v1/receipts              List proof bundles submitted to this tenant (as a collector)
GET       /v1/receipts/:id          Full stored bundle for one submitted receipt
...
- Timestamp must be within 5 minutes of server time (`auth.rs::replay_tolerance_secs()`, default 300 — override with `HSIP_REPLAY_TOLERANCE_SECS` env var, same pattern as `RATE_LIMIT_RPM`) or `401`.
...
| `messages` | id, tenant_id, content, signature, timestamp | `content` encrypted at rest with ChaCha20-Poly1305 (domain-separated key from `identities.signing_key_b64`'s encryption) — see Field Encryption at Rest below. |
...
| `credentials` | id, tenant_id, claim, user_token, issuer_verify_key, signature, revoked | `claim`/`user_token` encrypted at rest — see Field Encryption at Rest below. |
...
| `submitted_receipts` | id, collector_tenant_id, submitter_label, receipt_type, source_tenant_id, source_record_id, bundle_json, valid, submitted_at | `UNIQUE(collector_tenant_id, receipt_type, source_tenant_id, source_record_id)`. A collector's inbox of already-verified proof bundles submitted by other, independent HSIP instances — see Receipt Collection below. Never holds decision payload content or private key material. |
...
## Live Public Deployment (Railway)

A real, persistent instance is running at **`https://hsip-1phase-production-c45d.up.railway.app`** — a separate Railway service from Predicta's own deployment, in the same Railway project (`daring-motivation`), plus a Railway Postgres addon for `DATABASE_URL` so state survives redeploys. This is the instance `docs/SANDBOX_QUICKSTART.md` and the README's "Try it live" link point at, and the one Predicta's production `HSIP_API_KEY`/`HSIP_API_URL` are wired to (see Trading Bot Integration below — that integration is now live, not just designed).

**Variables set on the HSIP Railway service** (`railway.toml`'s own commented-out template documents the intent; these are the values actually in effect):
- `HSIP_ADMIN_KEY` — a fixed, generated-once key (`openssl rand -hex 32 | sed 's/^/hsip_/'`). Without this, `bootstrap_admin` mints a brand-new random admin key on every container restart and only prints it to that boot's logs — there is then no stable way back into the instance at all, since the DB (see next point) resets too.
- `DATABASE_URL` — a **reference** (`${{Postgres.DATABASE_URL}}` in Railway's UI, not a copy-pasted connection string) to the Postgres addon. Without it, `hsip-api`'s Dockerfile defaults `HOME=/data` but that's still the container's own ephemeral filesystem — every tenant, key, and decision would be wiped on every redeploy.
- `HSIP_SANDBOX=true` — enables the public, unauthenticated `POST /v1/sandbox/provision` self-serve 24-hour trial key flow (see Sandbox docs above). This is a public demo instance by design; production HSIP deployments should leave this unset.
- `HSIP_PUBLIC_URL` — set to the instance's own Railway URL, used for CORS/self-links.

**Operational gotcha that cost real debugging time, worth not repeating:** Railway's per-service Variables panel *stages* edits — the "Apply N changes" / "Deploy" banner has to be clicked explicitly. Using the `⋮` menu's "Redeploy" on an *existing* deployment redeploys that deployment's already-baked config, silently ignoring any staged-but-undeployed variable changes. Symptom seen in practice: `HSIP_SANDBOX=true` was staged and saved, but the running container's boot log still showed `Configuration: desktop defaults (/data/.hsip)` and a fresh "First-time setup" admin key — because the visible "Active" deployment predated the staged changes. Always confirm the fix by reading the *new* deployment's own Deploy Logs for the expected boot lines (`Using admin key from HSIP_ADMIN_KEY env var`, the `HSIP_SANDBOX=true — UNAUTHENTICATED PROVISIONING IS ON` banner, `✓ Database initialized: Postgres` not `SQLite`), not by re-testing against whatever's already running.

**Predicta's key on this instance:** a dedicated `ai_agent`-type key, minted via the dashboard's "+ Connect" button on the AI Decisions page while signed in with `HSIP_ADMIN_KEY` — never the admin key itself. See Trading Bot Integration below.
...
# Submit a local decision/audit proof to a remote collector — see Receipt Collection below
hsip receipts submit <decision-or-audit-id> --label <name> --collector-url <url> --collector-key <key> [--type decision|audit] [--api-url URL] [--key K]
```
...
crates/hsip-cli/src/commands/
  ...
  receipts.rs   ReceiptsCmd enum: Submit — fetches a local decision/audit proof bundle and submits it to a remote collector
...
**Windows gotcha, found by a real user's `cargo test`:** `load_client_verifier_accepts_a_real_ca_certificate`'s helper shells out to the system `openssl req -x509 ...` to generate a throwaway CA — some `openssl.exe` builds (seen with a portable/MSYS2-built binary on Windows) have a build-machine-specific default config path baked in (e.g. `Z:/extlib/.../openssl.cnf`) that doesn't exist on the machine actually running the test, so `openssl req` exits non-zero with `BIO_new_file: No such file or directory` and the test fails via `assert!(status.success(), "openssl CA generation failed")` — nothing to do with the crate's own TLS code. Fixed by setting `OPENSSL_CONF=""` on the child process in `generate_test_ca()`: an empty `OPENSSL_CONF` makes `openssl` skip config-file lookup entirely and fall back to compiled-in defaults, which this `-subj`-supplied self-signed cert doesn't need a config file for anyway. Confirmed unaffected on Linux (OpenSSL 3.0.13) — all three `mtls::tests` still pass, including 5 repeated runs of the full `hsip-api --lib` suite with no flakes.
...
## Field Encryption at Rest (`messages.content`, `credentials.claim`/`user_token`)

Per-transaction key derivation (§4.29 in THREAT_MODEL.md) explicitly did not protect against an attacker who already has read access to this database — signing-key material was already encrypted, but everything else (message content, credential claims) sat in plaintext columns, readable by anyone who could open the SQLite file or dump the Postgres tables. This closes that for the two clearest cases of genuinely sensitive free text in the schema.

**Why these two fields, not "encrypt everything":** `messages.content` (arbitrary signed message text) and `credentials.claim`/`credentials.user_token` (verifiable-credential claim data) are both standalone — no other code path needs to read, index, filter, or hash-chain over their plaintext value. `audit_entries.details` was deliberately **not** touched in this pass: it's entangled with the BLAKE3 hash chain (`audit_log::compute_entry_hash` hashes it), and encrypting the stored column while keeping `GET /v1/audit/verify-proof`'s pure, DB-free, caller-supplied-plaintext verification contract intact needs a "hash the plaintext, encrypt only what's persisted" design that wasn't worth rushing into the same pass as this — see "Remaining" below.

**Why SQLCipher/full-database encryption wasn't the approach:** HSIP's `sqlx::Any` backend spans both SQLite and PostgreSQL (see SQLite → PostgreSQL Migration below) — a SQLite-specific extension wouldn't cover Postgres deployments, and would be a second, backend-specific encryption mechanism alongside the one already used for signing keys. Application-level field encryption, reusing the exact same ChaCha20-Poly1305 + HKDF-SHA256-from-master-key primitive `key_encryption.rs` already uses for signing keys, works identically on both backends with zero new dependencies.

**Implementation:**
- `crates/hsip-api/src/key_encryption.rs` — `encrypt_field`/`decrypt_field`. Same nonce(12)‖ciphertext+tag Base64 format as `encrypt_signing_key`/`decrypt_signing_key`, but a **domain-separated** derived key (`derive_field_encryption_key`, HKDF info `"hsip-field-encryption-v1"` vs. the signing-key path's `"hsip-key-encryption-v1"`) — a bug specific to one call site's usage pattern can't be leveraged against the other, even though both ultimately derive from the same master key.
- `routes/messages.rs::sign()`/`verify()` — sign the request's plaintext `content` exactly as before (encryption never touches what gets signed), then encrypt it for storage right before the `INSERT`. `list()` decrypts after `SELECT`, before returning to the authenticated caller.
- `routes/credentials.rs::issue()` — signs the plaintext canonical JSON (unaffected — `canonical_json` reads from the request, not the DB), encrypts `claim`/`user_token` before the `INSERT`. `list()` decrypts both after `SELECT`. `verify()` needed **no changes** — it already verifies against the caller-supplied `credential` in the request body, never reading `claim`/`user_token` back from the database at all.
- **A real leak found by the new test, not hypothetical:** `issue()`/`verify()` both passed the plaintext `req.claim`/`req.credential.claim` as the `details` argument to `audit_log::record_best_effort` — meaning even after `credentials.claim` itself was encrypted, the same plaintext still landed in `audit_entries.details` on every issue/verify, silently undoing the confidentiality just added. Fixed by logging the credential ID instead (`revoke()` already did this correctly — the same fix, applied consistently). `messages.rs`'s own audit calls only ever logged `peer_verify_key`, never `content`, so no equivalent fix was needed there.
- `bin/hsip_migrate.rs` needed **no changes** — it copies whatever bytes are actually stored, verbatim, regardless of whether the application layer encrypted them first.

**Verified:** 4 new unit tests in `key_encryption.rs` (roundtrip, wrong master key fails, tampered ciphertext fails, domain separation — a field-encrypted value must not decrypt as a signing key even under the same master key). New integration test `test_message_content_and_credential_claim_are_encrypted_at_rest` proves the actual security property, not just that the API still round-trips: signs a message and issues a credential with distinctive content, confirms the API still returns exact plaintext, then reads the **raw DB row directly** and asserts the plaintext substring is absent and the stored value decrypts correctly under the real master key — plus a check that `audit_entries.details` for that same operation never contains the claim text either (the leak above, now closed). Also verified against a real running server: signed a message and issued a credential with distinctive marker strings, confirmed the API returned them correctly, then `grep`bed the raw `.hsip/hsip.db` and `.hsip/hsip.db-wal` files directly — zero occurrences of any plaintext marker in either file, both before and after the audit-log leak was found and fixed (the leak reproduced against the real running server first, confirming it wasn't a test-only artifact, then reproduced-fixed the same way).

**What this does not do:** `audit_entries.details` remains plaintext (see above); `decisions.payload_hash` was never plaintext to begin with (HSIP never receives decision content, only its hash — see Decision Attestations); `consents`/`contacts`/`trusted_peers` store public keys and labels, not secrets. An attacker with the master key *and* the database can still decrypt everything — this protects against database compromise alone, not master-key compromise, which remains the single highest-value target (see THREAT_MODEL.md's Trust Boundaries table).

---

## Receipt Collection (`/v1/receipts/*`)

Field encryption at rest (above) protects a single HSIP instance's own database against compromise. It does nothing about the much larger question a business actually deploying HSIP across many people/agents runs into: does everyone's raw operational data (messages, credentials, consents, decisions) have to live in one shared, centrally-reachable database at all? A single server holding every employee's or agent's full HSIP database is exactly the kind of large, valuable, network-reachable target the rest of this document has been hardening against one gap at a time — the better answer is for that target not to exist.

**The design:** every person/agent keeps running their own HSIP instance purely locally (desktop mode, `127.0.0.1:7474`, never exposed to a network) — nothing changes about how HSIP already works day to day. When centralized audit visibility is genuinely needed, that instance submits only the already-signed, self-contained *proof bundle* for a specific decision or audit entry (exactly what `GET /v1/decisions/:id/proof` / `GET /v1/audit/:id/proof` already return — hashes, an Ed25519 signature, a public verify key, and, once anchored, a Merkle inclusion proof) to a separate "collector" HSIP instance. The collector never receives the submitting instance's database, private keys, message content, credential claims, or consent records — only a small, independently-reusable proof that a specific signed event happened. Compromising the collector discloses which decisions were made and by which already-public verify keys, not the sensitive operational data those decisions concerned, because the collector never had it.

This is not a new network protocol or a new trust primitive — it's the existing "pure, DB-free, auth-free verification function" pattern (`routes::decisions::verify()` / `routes::audit::verify_proof()`, the same functions a regulator or acquirer's engineering review would run independently) reused as the collector's own gatekeeper for what it's willing to store.

**Implementation:**
- `db.rs` — new `submitted_receipts` table: `id`, `collector_tenant_id`, `submitter_label` (caller-supplied, informational only — not a verified identity claim), `receipt_type` (`decision`\|`audit`), `source_tenant_id`, `source_record_id`, `bundle_json` (the verbatim submitted bundle), `valid` (0/1 — always 1 in practice, since an invalid bundle is rejected before the `INSERT` and never stored), `submitted_at`. `UNIQUE(collector_tenant_id, receipt_type, source_tenant_id, source_record_id)` makes re-submitting the same receipt to the same collector idempotent-safe (`409 Conflict`) rather than silently duplicating storage. Added to `bin/hsip_migrate.rs`'s `TABLES` list per the standing invariant.
- `crates/hsip-api/src/routes/receipts.rs`:
  - `submit()` — `POST /v1/receipts/submit`. Deserializes the caller-supplied `bundle` into the exact same `VerifyDecisionRequest`/`VerifyAuditProofRequest` shape `routes::decisions::verify`/`routes::audit::verify_proof` already accept, and calls those functions **directly as ordinary async functions** (not over HTTP — same process, same request) to independently re-verify the bundle before ever storing it. A bundle that fails to deserialize, or verifies `false`, is rejected `400` and never reaches the `INSERT` — this table is meant to be a trustworthy log of genuinely-verified receipts, not a raw inbox of caller claims. Writes a `receipt.submitted` audit entry via `audit_log::record_best_effort` (never `?` — see the standing invariant on audit-write failure handling).
  - `list()` — `GET /v1/receipts`. Summaries only, newest first, scoped to the calling collector tenant.
  - `get_one()` — `GET /v1/receipts/:id`. Full detail including the original bundle, for a deeper audit.
  - All three routes take `TenantId` — a collector is just an ordinary HSIP instance/tenant whose owner chooses to accept submissions from other, independent instances; there's no special "collector mode" to configure.
- `crates/hsip-cli/src/commands/receipts.rs` — `hsip receipts submit <id> --type decision|audit --label <name> --collector-url <url> --collector-key <key> [--api-url URL] [--key K]`. The client-side half: fetches the proof bundle from *this* machine's own local instance (using the normal local key/URL resolution), then `POST`s it to the remote collector using a separate, collector-scoped bearer key (`--collector-key` / `HSIP_COLLECTOR_KEY`) — the two credentials are deliberately distinct, since the local key authenticates to this machine's own instance and the collector key authenticates to somebody else's. Prints the collector's confirmation (receipt ID, source tenant/record ID, verified status).

**Verified against two real, separately-running `hsip-api` processes**, not just the integration tests — this is the one property worth confirming end-to-end rather than trusting unit-level reasoning about: booted a "source" instance and a separate "collector" instance on different ports with separate data directories (so they have genuinely independent SQLite databases, not a shared or simulated one), recorded a real decision on the source instance, submitted its proof via the actual compiled `hsip-cli receipts submit` binary, and confirmed: the collector's `GET /v1/receipts` listed exactly one entry with the correct source tenant/record ID; the collector's own `GET /v1/decisions` returned `[]` the entire time — proving the collector genuinely never received the source instance's decision content, database, or any tenant data beyond the one proof bundle; a duplicate submission of the same receipt was rejected `409`; and a bundle with a tampered signature was rejected `400` and never appeared in the collector's list. Also covered by two new integration tests (`test_receipts_submit_list_get_and_reject_invalid`, `test_receipts_submit_audit_entry_receipt`) using two separate `test_app()` instances to model the source/collector split, exercising both `receipt_type`s plus the duplicate/tamper/invalid-type rejection paths.

**What this does not do:** it doesn't make server-mode HSIP deployments (a shared multi-tenant instance on a network, e.g. `config.toml` + `[server.tls]`) unnecessary or wrong for every business — some businesses genuinely need shared, live operational data (e.g. a shared consent registry multiple services query in real time), and that's exactly what server mode with mTLS already supports. Receipt collection is specifically for the common case where the *only* thing that needs to be centralized is auditability of what happened, not the operational data itself. It also doesn't verify `submitter_label` as an identity claim — that's informational metadata; the actual cryptographic identity behind a decision receipt is `accountable_key`/`issuer_verify_key` inside the bundle, which *is* verified. See THREAT_MODEL.md §4.31.
...
**Windows/MSVC gotcha, found by a real user's `cargo build --workspace`:** `z3-sys`'s `cmake --build ... --parallel 8` step can fail with `fatal error C1060: compiler is out of heap space` — MSVC running several of Z3's large, template-heavy `.cpp` translation units (e.g. `sat_lookahead.cpp`, `macro_util.cpp`) in parallel exhausts available memory on machines without a lot of RAM to spare. This is specific to the MSVC toolchain (`cl.exe`'s per-process heap is more memory-hungry compiling this code than GCC/Clang, which is why the Linux/macOS sandbox this repo was originally developed in never hit it) and specific to the very first clean build of `hsip-verify` in a given `target/` directory — once `z3-sys`'s compiled artifacts are cached, incremental builds don't recompile Z3 at all.

Two ways to work around it, neither requiring a repo change:
- **Skip it entirely for routine dev/test loops** — scope commands to the crates actually being changed instead of `--workspace`/`--lib` across everything, e.g. `cargo build -p hsip-api -p hsip-cli`, `cargo test -p hsip-api`, `cargo clippy -p hsip-api -p hsip-cli -- -D warnings`. `hsip-verify` is a leaf crate nothing else in the workspace depends on, so this never needs to touch Z3 at all.
- **If a `--workspace` build genuinely is needed**, cut Cargo's build parallelism so MSVC isn't running many heavy compiles at once: `cargo build --workspace -j 2` (drop to `-j 1` if it still runs out of heap). This is a one-time cost per clean `target/`, same as the ~8-minute Z3 compile documented above — Cargo caches the result afterward.
...
**Per-transaction signing key derivation:** every decision used to be signed directly with the tenant's long-lived root identity key (`identities.signing_key_b64`) — meaning that key itself touched live signing on every request, and every decision from the same tenant carried the exact same public key, trivially linkable by anyone who saw two of them. `hsip_core::tx_key::derive_transaction_signing_key(root_seed, tenant_id, decision_id)` closes both: HKDF-SHA256 (`salt = tenant_id`, `info = "hsip-tx-key-v1|" + decision_id`) deterministically derives a fresh, single-use Ed25519 key per decision from the root seed. Two properties, together:
- **Unlinkable to an outside observer.** A derived key is statistically indistinguishable from random without `root_seed` — two decisions from the same tenant produce completely unrelated-looking public keys on the wire, and the root itself never signs anything directly.
- **Re-derivable and verifiable by the root holder.** HKDF is deterministic — the same `(root_seed, tenant_id, decision_id)` always produces the same key, so a tenant (or an auditor given temporary access to that tenant's root seed) can independently recompute any past decision's signing key and confirm it really descends from that tenant's root, with no extra secret or storage on HSIP's side beyond the tenant's own already-encrypted identity row. `hsip_core::tx_key::verify_transaction_key_derivation` is the matching audit-side check.

**What this does not do:** it does not make a tenant's own decisions unlinkable to someone with access to HSIP's own database — `tenant_id` sits right next to every decision there regardless, for ordinary multi-tenant operation (auth, rate limiting, queries). The unlinkability property is specifically against an outside network observer or anyone who only ever sees individually-published signed artifacts, not against database access. See "How to Avoid Database Compromise" below for that separate problem.

- `record()` derives the key from `decision_id` *inside* the chain-retry loop (not once per request) — each retry attempt generates a fresh `decision_id`, so it needs its own derived key too. `issuer_verify_key` in the response is now this derived key's public bytes, not the tenant's static identity key.
- `db.rs` — `decisions.issuer_verify_key TEXT`, nullable (ignored-error `ALTER TABLE ADD COLUMN`, same pattern as `accountable_key_signature`). `NULL` means the decision predates this scheme and was signed directly with the tenant's root key — `proof()` falls back to `identities.verify_key_b64` for those rows, exactly as every decision worked before this. Added to `bin/hsip_migrate.rs`'s `TABLES` list per the standing invariant.
- `verify()` needed **no changes at all** — it already took `issuer_verify_key` as an explicit request field and verified the signature against whatever key was supplied, never assuming it equals the tenant's stored identity key. That existing design is what made this change possible without touching the wire format's verification contract.
- **Verified:** unit tests in `hsip_core::tx_key` (determinism, domain separation by tenant and by decision_id, unrelated output from unrelated roots, the audit-side check correctly accepting the real derivation and rejecting every wrong input). `tests/integration.rs::test_decision_keys_are_per_transaction_derived_and_root_never_signs_directly` — two decisions from the same tenant get different keys, neither equals the root identity key, both independently verify through the real DB-free `POST /v1/decisions/verify`, and — recomputing directly against the test database's own decrypted root seed — both derived keys are exactly reproducible via `derive_transaction_signing_key`, while a wrong `decision_id` reproduces neither. Also verified against a real running server: two recorded decisions came back with two different `issuer_verify_key` values, both different from the tenant's root `verify_key`, and both verified `true` through `/v1/decisions/verify`.
...
- `crates/hsip-core/src/tx_key.rs` — `derive_transaction_signing_key`/`verify_transaction_key_derivation`, HKDF-SHA256. No I/O — pure function of its inputs, same discipline as `merkle.rs`/`canonical.rs`.
- `crates/hsip-api/src/routes/decisions.rs`:
  - `record()` — `POST /v1/decisions`. Resolves the authenticated `api_keys` row, validates fields, chains to the tenant's last decision via `prev_hash`, derives a per-decision key from the tenant's root seed and signs `event_hash` with it (see above — not the root key directly). Retries on `UNIQUE(tenant_id, prev_hash)` conflict (another request extended the chain first) up to `MAX_ATTEMPTS`. Writes `decision.recorded` audit entry.
  - `list()` — `GET /v1/decisions`.
  - `proof()` — `GET /v1/decisions/:id/proof`. Returns the full self-contained bundle, reading the decision's stored `issuer_verify_key` (falling back to the tenant's root identity key for pre-migration rows). If unanchored yet, `anchored: false` with signature-only proof. If anchored, reconstructs the batch's leaf set and regenerates the inclusion proof on demand (not stored — recomputed from `decisions.anchor_id`/`merkle_index`).
...
## Trading Bot Integration (Predicta) — Live

Predicta is the repo owner's own AI trading-decision system, used since early in this project as the motivating example for the decision-attestation design (`accountable_key`, the DB-free `POST /v1/decisions/verify`, the whole "HSIP never sees the actual trade" model exists because of it — see Decision Attestations above). This started as a design reference; it is now a real, live, verified integration.

**Status: DONE, verified end-to-end against the live Railway deployment (see "Live Public Deployment" above).** Predicta's own repo already had this fully built (`fetchers/hsip_client.py`, wired into `execute_high_value_trade`/`close_high_value_trade`/`execute_low_value_trade`/`close_low_value_trade`/`POST /trade/smart-order` in Predicta's codebase) — the only missing piece was infrastructure, not code: Predicta's Railway service needed a real, reachable `HSIP_API_URL` and a dedicated `HSIP_API_KEY`, both now set (see Live Public Deployment above). Confirmed via HSIP's own AI Decisions dashboard page: a real human-authorized Buy on Predicta showed up within seconds, tagged "Predicta · AI agent," with a "✓ Locked & verified / Genuine and untampered" status. This is the everyday case Steps 1-5 below describe, now actually happening rather than hypothetical — kept below as the reference walkthrough, not as an open TODO.
...
### Resolved (was "Open questions to resolve once Predicta's repo is added")

- **Language:** Python. Predicta's `fetchers/hsip_client.py` calls the raw HTTP API directly via `requests` — it does not import HSIP's official `sdks/python/hsip/client.py` `HSIPClient` class, it's a small independent connector with the same fail-safe conventions as Predicta's other optional integrations (FRED, Finnhub). Worth reconciling with the official SDK later, not urgent — the wire contract is identical either way.
- **Identity:** Predicta had no prior Ed25519 identity; `_get_identity()` auto-creates one on first call via `POST /v1/identity` and caches it in-process for the life of that Predicta run.
- **Decision types:** Predicta only ever attests `"buy"`/`"sell"` — **never `"hold"`**, because `attest_transaction()` is called only on a real, already-*executed* order (see "Authority model" below), and "hold" isn't a transaction. `strategy_id` varies by engine/path: `high_value_intraday`, `high_value_intraday_manual`, `low_value_swing`, `low_value_swing_manual` — Predicta genuinely has more than one distinct decision type worth separately attesting, one per engine × automated-vs-manual-execute path.
- **Network topology:** Predicta runs on its own separate Railway service, not on the same machine as HSIP — `HSIP_API_URL` points at HSIP's live Railway URL (see Live Public Deployment above), not `127.0.0.1`.

### Authority model — human authorizes, AI decides (by explicit design, not a limitation)

Predicta's automated scans only ever analyze and log a *hypothetical* candidate — they never place a real order autonomously. The only way a real trade happens, and therefore the only way `attest_transaction()` ever fires, is a human clicking Buy/Sell on a dashboard candidate. This is a direct, explicit requirement from Predicta's own owner ("that power belongs to me, Predicta only analyze and predict, I do the action of buying and sell" — see Predicta's own CLAUDE.md) — Predicta's automated scans are advisory only.

This is not in tension with calling it an "AI decision" on HSIP's AI Decisions page: the `accountable_key` behind the attestation is Predicta's own identity, cryptographically binding *Predicta's analysis* to the recorded decision — the human click is a separate authorization gate on top of it, not the thing being attested. If Predicta is ever given authority to execute autonomously, the exact same `attest_transaction()` call fires unchanged; nothing on HSIP's side would need to change.
...
## DNS Resolver Response Spoofing Fix (`hsip-dns`)

A QA pass against "what will users misunderstand / which decision depends on memory instead of design" turned into an actual security-lens sweep of `hsip-dns` — a crate that had gone untouched by every prior QA round this session. It's the one part of HSIP whose entire purpose is "trust this less than the raw internet," which made an unvalidated forwarding path in it worth a closer look than its "Working, not actively flagged" status had gotten.

**Found:** `handle_query`'s upstream-forwarding path (`crates/hsip-dns/src/lib.rs`) bound an ephemeral `UdpSocket` on `0.0.0.0:0` (not loopback-only), sent the client's query to `1.1.1.1:53` via `send_to`, then called `recv_from` and forwarded whatever came back to the client — discarding the response's source address entirely and never checking that the response's transaction ID matched the query's. An unconnected UDP socket accepts a datagram from *any* source, so any attacker able to reach that ephemeral port (on a shared network, or via a MITM anywhere on the path to `1.1.1.1`) could race the real upstream with a spoofed response and have it relayed straight to the client — classic DNS response spoofing / cache poisoning, defeating the entire point of running a "security" DNS resolver for domains that aren't even on the tracker blocklist.

**Fixed**, two layers:
1. The forwarding socket now calls `.connect(upstream)` before sending, so the OS itself refuses to deliver any datagram whose source doesn't match `1.1.1.1:53` — the primary fix, enforced by the kernel rather than application logic.
2. `response_transaction_id_matches()` — a second, cheap check that the response's first two bytes (transaction ID) match the query's, dropping and logging a warning on mismatch. Defense-in-depth against response confusion even from the correctly-connected peer (e.g. a genuinely misbehaving or compromised upstream).

**Verified three ways, not just read as code:**
- Unit test for the transaction-ID check (pure function, no networking).
- `connected_udp_socket_ignores_datagrams_from_other_sources` — proves the actual OS-level primitive the fix depends on, using the same `bind`/`connect`/`send`/`recv` calls production code uses: an attacker socket's spoofed datagram sent directly at the connected forwarding socket's address is never delivered, while the real connected peer's reply is.
- `handle_query_forwards_a_real_upstream_response_to_the_client` — end-to-end through the real handler with a fake upstream, proving the fix didn't break legitimate forwarding.
- Against a real running server: `POST /v1/dns/enable`, then a raw Python DNS query for a tracker domain (`doubleclick.net`) got a correct NXDOMAIN with the right transaction ID back with zero network dependency, and a raw query for `example.com` was actually forwarded to and answered by the real `1.1.1.1`, transaction ID matching — confirming the `connect()`-based fix works against a genuine, live upstream, not just a test double. `GET /v1/dns/status`/`log` afterward showed `queries_total: 2, blocked_total: 1` and the correct log entry, matching exactly what happened.
...
- **Never write `let _ = audit_log::record(...).await;` or `audit_log::record(...).await?` at a call site where the operation being audited has already committed.** Use `audit_log::record_best_effort(...)` instead — it logs via `tracing::error!` and increments `metrics::AUDIT_WRITE_FAILURES` on failure rather than discarding or propagating the `Result`. Found as a real bug at 9 call sites using the `let _ =` (silent) shape (`routes/admin.rs`, `routes/keys.rs`, `auth.rs` — §4.26), then at 14 more using the `?` (loud-but-wrong) shape (`routes/decisions.rs` plus `consent`/`credentials`/`identity`/`messages`/`sandbox`/`trust` — §4.27, found only once this codebase had a test that created genuine concurrent contention instead of testing sequentially). Both shapes are wrong the same way: a missing audit entry for an operation that otherwise succeeded is invisible to `GET /v1/audit/verify` either way, and `?` additionally turns a successful operation into a confusing `500` for the caller. The only sanctioned exceptions are `anchor_job.rs`'s two background-job sites, which correctly keep `?` — there's no live HTTP caller there to mislead, and the periodic anchor loop already logs and retries on the next tick.
- **Any new route handler that writes to the database, and then separately writes an audit entry describing that write, must use `audit_log::record_best_effort()` for the audit call, never `?` or `let _ =`.** This is now the default for every handler in this codebase (see the invariant above) — a new handler that gets this wrong reintroduces a bug class that took two separate QA passes to find and fix everywhere else.
- **A concurrency claim (a retry loop, a `UNIQUE` constraint meant to serialize concurrent writers, an optimistic-locking pattern) is not actually tested by a sequential test, no matter how many sequential cases it covers.** `tests/integration.rs::test_concurrent_decision_writes_do_not_fork_the_chain` is the only test in this codebase that creates genuine concurrent contention (via `tokio::spawn`, not sequential `.await`s) — it's what found the 14-site bug above, which every prior sequential test had missed for the entire life of this codebase. When adding a new concurrency-dependent invariant, add a test in this shape, not another sequential one that happens to call the same code path.
- **Two tests that both exercise a Prometheus metric must not share a label value if either asserts an exact before/after delta.** `metrics::AUDIT_WRITE_FAILURES` and every other metric are process-wide globals shared by every `#[tokio::test]` in the same test binary, which Rust runs in parallel by default — found as a real, genuinely flaky failure in `audit_log.rs`: `record_best_effort_does_not_panic_on_failure` and `record_best_effort_increments_failure_metric_on_failure` both drove a failing write through the `"key.created"` label, so running concurrently let one test's increment land inside the other's `before`/`after` snapshot window, intermittently breaking the exact-delta assertion (`after == before + 1.0`). A real user hit this on a real machine; it wasn't reproducible by re-running the single failing test in isolation, only the full suite, which is exactly why it went unnoticed by every prior individual test run. Fixed by giving each test in that module a distinct action-label string. When writing a metric-asserting test, either give it a label no other test in the same binary uses, or assert a *relative* change scoped to a value only that test could have produced.
...
| Test-suite QA pass (6 fixed lenses: which test proves nothing, which behavior has never been tested, which bug could all tests still miss, what randomness isn't covered, which production behavior can't be reproduced locally, which assumption is impossible to unit test) — wrote the first genuinely-concurrent test in this codebase's history (`test_concurrent_decision_writes_do_not_fork_the_chain`, 25 real `tokio::spawn`ed `POST /v1/decisions` calls at one tenant, not sequential `.await`s) to actually exercise the `UNIQUE(tenant_id, prev_hash)` retry loop's concurrency claim instead of trusting sequential tests that structurally can't create the race it exists to handle. It immediately found a real bug: `routes::decisions::record`'s own audit write used `.await?` on an already-committed primary operation, so a downstream audit-write failure under real contention returned a confusing `500` for a decision that had actually already been recorded — the same failure shape §4.26 had just fixed elsewhere, just missed here because nothing had ever exercised the contention that reveals it. Swept for the identical pattern and found it at 13 more sites across `consent`/`credentials`/`identity`/`messages`/`sandbox`/`trust` — all 14 now use `record_best_effort()`. Also caught and fixed the new test's own first-draft bug (asserting every concurrent write returns `200`, which real contention correctly violates via `409` once `MAX_ATTEMPTS` is exhausted) before trusting it. Verified against a real running server the same way as §4.26: sabotaged `audit_entries` mid-flight, confirmed `POST /v1/consent/grant` still succeeded while the failure was logged. See THREAT_MODEL.md §4.27 | `routes/decisions.rs`, `routes/consent.rs`, `routes/credentials.rs`, `routes/identity.rs`, `routes/messages.rs`, `routes/sandbox.rs`, `routes/trust.rs`, `tests/integration.rs` |
| Restored the documented `cargo clippy --workspace -- -D warnings` quality gate to green — it had been silently failing for the entire life of this session (and, per lack of any prior fix commit, likely long before), because `hsip-core` itself failed to compile under `-D warnings` (an unneeded `mut` in `keystore.rs`, a redundant closure in `session.rs`, a `SecureString::from_str` name colliding with the `std::str::FromStr` trait method in `secure_memory.rs`), which short-circuited the workspace build before clippy ever reached any downstream crate. Fixing those 3 and re-running repeatedly surfaced 23 more real, previously-invisible clippy errors one crate-boundary at a time across `hsip-net`, `hsip-common`, `hsip-api` (both the `lib` and, separately, the `bin "hsip-api"` target — `main.rs` compiles `db.rs`/`state.rs` a second time via its own `mod` tree, so the two targets' dead-code analysis disagreed on `db::init`/`AppState::new`, both genuinely used only by the `lib` target's own tests), `hsip-cli`, `hsip-intercept`, `hsip-verify`, and `hsip-telemetry-guard` — mostly mechanical (`or_insert_with(Vec::new)` → `or_default()`, manual `strip_prefix`, derivable `Default` impls, `manual_is_multiple_of`, needless borrows, unused imports/variables) with two genuine dead-field removals in `hsip-telemetry-guard` (`ConsentGate::anti_replay` and `PolicyEngine::regex_cache` — both constructed but never read; the crate's real anti-replay enforcement already lives entirely in `consumed_tokens`/`consume()`, unrelated to the unused `AntiReplayGuard` field). No behavior changes anywhere — every touched crate's existing test suite (hsip-core: 74 unit + 9 integration; hsip-net: 11 unit + 1 integration; hsip-common: 46 unit; hsip-telemetry-guard: 53 unit) still passes, plus `cargo test --workspace` run in full afterward. `hsip-net`'s pre-existing `#[allow(clippy::too_many_arguments)]` precedent (`udp.rs::handle_control_message`) was reused verbatim for the newly-flagged `process_control_messages`, rather than introducing a new pattern | `hsip-core/src/{keystore,session,secure_memory}.rs`, `hsip-net/src/{hello,udp,input_validator}.rs`, `hsip-common/src/quantum_physics/{decoherence,entanglement,observer_effect,uncertainty}.rs`, `hsip-api/src/{db,state,main,metrics,mtls}.rs`, `hsip-api/src/routes/{proxy,sandbox}.rs`, `hsip-cli/src/commands/{agent,trust}.rs`, `hsip-intercept/src/{privacy,router,overlay}.rs`, `hsip-intercept/src/linux/{event_monitor,overlay}.rs`, `hsip-verify/src/models.rs`, `hsip-telemetry-guard/src/{consent_gate,flow_meta,known_endpoints,policy}.rs` |
| Stood up the first real, persistent public deployment — `https://hsip-1phase-production-c45d.up.railway.app` on Railway, with a Postgres addon (`DATABASE_URL` as a live reference, not a copy-pasted string) so tenants/keys/decisions survive redeploys, `HSIP_ADMIN_KEY` fixed so the admin credential doesn't regenerate on every restart, and `HSIP_SANDBOX=true` for the public 24-hour self-serve trial key flow. See "Live Public Deployment" above for the full variable list and the Railway staged-changes-need-an-explicit-Deploy-click gotcha found while setting it up. Linked from the README's new "Try it live" section, the landing page's hero CTA (`docs/index.html`), and `docs/SANDBOX_QUICKSTART.md` (previously pointed at a placeholder `demo.hsip.io` domain that was never real — fixed to the actual live URL) | `railway.toml` (pre-existing, now actually used), README.md, docs/index.html, docs/SANDBOX_QUICKSTART.md, CLAUDE.md |
| Predicta integration (see Trading Bot Integration above) went from "designed, not started" to live and verified — Predicta's own repo already had the client-side half fully built; the only missing piece was network reachability (Predicta's Railway service needed a real `HSIP_API_URL`/`HSIP_API_KEY` pointed at the deployment above, not `127.0.0.1`). Confirmed via a real human-authorized Buy on Predicta's dashboard showing up on HSIP's AI Decisions page within seconds, correctly tagged "Predicta · AI agent" with a genuine/verified status | (Predicta-side: `fetchers/hsip_client.py` and callers, pre-existing, unchanged) |
| Clarified the Messages page's copy — it visually reads like a live chat app (bubbles, a "Message X…" compose placeholder, a "Receive" inbox framing), but HSIP has no transport between separate instances; every message is signed and stored locally only, and delivery is entirely manual (copy the signed text, send it via whatever channel you already trust, the recipient pastes it back in to verify). Reworded the empty states, compose hint, and receive dialog to say that plainly instead of implying delivery happens — no mechanics changed | `dashboard/src/pages/Messages.jsx` |
...
- Test-suite QA pass against six fixed questions (which test proves nothing, which behavior has never been tested, which bug could all tests still miss, what randomness isn't covered, which production behavior can't be reproduced locally, which assumption is impossible to unit test) — wrote this codebase's first genuinely-concurrent test (`test_concurrent_decision_writes_do_not_fork_the_chain`, real `tokio::spawn`ed concurrent requests against one tenant, not sequential `.await`s) to actually test the `UNIQUE(tenant_id, prev_hash)` retry loop's concurrency claim, which every prior sequential test structurally could not. It immediately surfaced a real, previously-undiscovered bug: `routes::decisions::record`'s own audit write used `.await?` on an already-committed primary write, so a downstream audit failure under genuine contention returned a confusing `500` for a decision that had, in fact, already been recorded — the same failure shape §4.26 had just fixed elsewhere, missed here only because nothing had ever created the contention needed to reveal it. Swept for and fixed the identical pattern at 13 more sites across `consent`/`credentials`/`identity`/`messages`/`sandbox`/`trust`. See THREAT_MODEL.md §4.27.
- Restored `cargo clippy --workspace -- -D warnings` (a documented Quality command at the top of this file) to a genuinely passing state — it had never actually run to completion before, because `hsip-core` itself failed to compile under `-D warnings`, which silently prevented clippy from ever reaching any crate beyond the first few in the workspace. Fixed 3 errors in `hsip-core`, then repeatedly re-ran to find 23 more real errors one crate-boundary at a time across `hsip-net`, `hsip-common`, `hsip-api` (both its `lib` and `bin` targets, which turned out to compile `db.rs`/`state.rs` independently via two separate module trees), `hsip-cli`, `hsip-intercept`, `hsip-verify`, and `hsip-telemetry-guard` — including two genuinely-dead struct fields in `hsip-telemetry-guard` removed outright rather than lint-suppressed. All mechanical, zero behavior changes; every touched crate's full test suite still passes.
- DNS resolver response-spoofing fix (`hsip-dns`) — the one crate every prior QA round this session had skipped, since they all scoped to `hsip-api`. `handle_query`'s upstream-forwarding path accepted a reply from *any* UDP source (unconnected socket, source address discarded, no transaction-ID check) and relayed it straight to the client — classic DNS response spoofing, for a resolver whose entire purpose is reducing trust in the raw network. Fixed with a `connect()`-ed forwarding socket (OS-level source filtering, the primary fix) plus a transaction-ID match check as defense-in-depth. Verified with a unit test, a test proving the OS-level connect()-based filtering itself using the real production primitives, an end-to-end `handle_query` test against a fake upstream, and — against a real running server — a raw DNS query for a tracker domain got a correct NXDOMAIN with zero network dependency while a query for a non-blocked domain was genuinely forwarded to and answered by the real `1.1.1.1`. See THREAT_MODEL.md §4.28.
- Per-transaction decision-signing key derivation (`hsip_core::tx_key`) — every decision used to be signed directly with the tenant's long-lived root identity key, so every decision from a tenant carried the same public key (trivially linkable) and the root itself touched live signing on every request. New `derive_transaction_signing_key(root_seed, tenant_id, decision_id)` (HKDF-SHA256) deterministically derives a fresh, single-use key per decision instead — unlinkable to an outside observer without the root seed, but re-derivable and independently verifiable by anyone who does hold it (the tenant, or an auditor given temporary access), via the matching `verify_transaction_key_derivation`. `POST /v1/decisions/verify` needed zero changes — it already took `issuer_verify_key` as an explicit field rather than assuming it equals the tenant's stored identity key. Does **not** make a tenant's own decisions unlinkable *within HSIP's own database*, where `tenant_id` still links every record for ordinary multi-tenant operation — see THREAT_MODEL.md §4.29 for that distinction and the next item below for the DB-compromise question this raised. Verified with unit tests (determinism, tenant/decision-id domain separation, audit-check correctness) plus a new integration test and a real-running-server check confirming two decisions get two different, unrelated keys, both different from the root, both independently verifiable.
- Field encryption at rest for `messages.content` and `credentials.claim`/`user_token` (`key_encryption::encrypt_field`/`decrypt_field`) — the direct follow-up to §4.29 above: an attacker with read access to the database itself, not just the network, could previously read every message and credential claim in plaintext. Same ChaCha20-Poly1305 primitive already used for signing-key encryption, domain-separated via a different HKDF info string, applied at the application layer so it works identically on both SQLite and PostgreSQL with no new dependency. Found and fixed a real leak in the process: `credentials.rs`'s own audit-log calls were passing the plaintext claim as the audit `details` field, silently undoing the column encryption on every issue/verify — fixed by logging the credential ID instead, matching what `revoke()` already did correctly. Verified against a real running server: signed a message and issued a credential with distinctive marker text, confirmed the API still returns exact plaintext, then `grep`ped the raw `.hsip/hsip.db`/`hsip.db-wal` files directly and found zero occurrences of any plaintext marker in either. `audit_entries.details` deliberately left unencrypted this round — it's entangled with the BLAKE3 hash chain and needs its own hash-the-plaintext-encrypt-only-storage design; see THREAT_MODEL.md §4.30 and "Remaining" below.
- Receipt collection (`/v1/receipts/*`) — the follow-up to the database-compromise question above, at the architecture level rather than the field level: instead of centralizing every person's/agent's full HSIP database (a large, valuable, network-reachable target), each instance stays purely local and only submits already-signed, self-contained proof bundles (the same bundles `GET /v1/decisions/:id/proof`/`GET /v1/audit/:id/proof` already produce) to a separate "collector" instance when centralized auditability is actually needed. The collector independently re-verifies every submission via the existing DB-free `routes::decisions::verify`/`routes::audit::verify_proof` functions, called directly rather than duplicated, before ever storing it — a bundle that doesn't verify is rejected, not recorded. New `submitted_receipts` table, `routes/receipts.rs` (`submit`/`list`/`get_one`), `hsip receipts submit` CLI command. Verified against two real, separately-running `hsip-api` processes on different ports with independent databases: recorded a real decision on a "source" instance, submitted its proof via the actual compiled CLI, and confirmed the collector stored and listed it correctly while its own `decisions` table stayed empty the entire time — proving the collector genuinely never received the source's operational data — plus confirmed duplicate-submission (`409`) and tampered-signature (`400`, not stored) rejection against the live servers. See THREAT_MODEL.md §4.31.
- Architectural-resilience QA pass ("what if requirements double / distributed / offline / latency 10x / hardware changes / a dependency disappears") found two real, fixable gaps. `auth.rs`'s replay-protection tolerance window (`REPLAY_TOLERANCE_SECS`) was a hardcoded 300s constant with no env override, unlike `RATE_LIMIT_RPM` — fixed via `replay_tolerance_secs()` reading `HSIP_REPLAY_TOLERANCE_SECS`, same pattern, default unchanged. Separately, `mslnk` (a tiny, single-maintainer, version-0.1 crate used only for Windows shortcut creation) was identified as the single highest dependency-abandonment risk in the tree — replaced with direct calls to the real Windows Shell COM API (`IShellLinkW`/`IPersistFile`) via Microsoft's own `windows` crate, already a transitive dependency of this workspace's other Windows-targeted crates. Verified without a Windows machine available in this sandbox by cross-compiling a scratch crate with the identical code for `x86_64-pc-windows-gnu`, producing a real, correctly-linked PE32+ executable, and cross-checking every API call against the actual downloaded crate source rather than guessing. See THREAT_MODEL.md §4.32 for what remains genuinely unverified (real-Windows runtime behavior).
- Reasoning QA pass (auditing recent decisions themselves, not the code fresh — weakest evidence, skipped steps, competing explanations) found one real bug: §4.32's `create_shortcuts` treated `CoInitializeEx`'s `RPC_E_CHANGED_MODE` result as a plain failure, when it actually means "COM already initialized on this thread under a different concurrency model" — a real possibility since this code runs inside a `#[tokio::main]` multi-threaded runtime where the OS thread can be reused for unrelated COM-touching work later. Fixed by distinguishing all three real outcomes (`S_OK`/`S_FALSE` own a reference and must uninitialize; `RPC_E_CHANGED_MODE` doesn't and must not) instead of a two-way success/failure check. Also added an operator caution to `HSIP_REPLAY_TOLERANCE_SECS`'s doc comment — widening it widens the replay window it exists to close. Verified the same way as §4.32 (isolated cross-compiled scratch crate, real linked PE32+ executable). Deliberately did *not* rush a fix for the same pass's other re-confirmed finding (`GET /v1/audit/verify`'s unbounded, uncached full-chain recompute) — that touches the hash-chain verification path directly, and this codebase already established the right caution for exactly that category of change in §4.30's decision to defer `audit_entries.details` encryption rather than rush it. See THREAT_MODEL.md §4.33.
- A follow-up peer-review-style self-critique ("what would another engineer criticize first," "what would I wish I'd asked before a production failure") found a second real gap in the same file: every `write_shortcut(...)` call in `create_shortcuts` was `let _ = ...` — a genuine failure produced zero trace anywhere, and since the binary runs with `windows_subsystem = "windows"` (no console), a bare log line wouldn't have been visible to a real user either. `create_shortcuts` now returns a log fragment (`ok`/`FAILED: <reason>` per shortcut) that `maybe_self_install` folds into the existing `install.log` file rather than inventing a second reporting path. Verified the same way as the fixes above it — rebuilt in an isolated scratch crate, still cross-compiles and links to a valid Windows executable. See THREAT_MODEL.md §4.33.
- Documented why HSIP has no built-in AWS/cloud dependency as a deliberate design choice, not a gap (new subsection right under "What This Project Is") — a trust/audit layer sharing infrastructure with the cloud vendor it may need to independently attest about is a weaker guarantee, and this is the same reasoning that already shaped Receipt Collection. Also wrote `TESTING.md`'s new Part 1 — a full manual, run-it-yourself walkthrough of every feature built across this project against a real running server (identity through receipts through the Windows self-install path), plus an explicit answer to "do I need a second computer to test this" (mostly no — two processes on one machine covers receipts/RBAC/decisions; a second machine only adds real value for the still-unverified Windows shortcut code, mTLS over a real network hop, cross-platform SDK checks, the DNS resolver protecting an actual other device, and the browser extension on a differently-configured browser).

- `hsip up` now prints a federated-trust onboarding hint after the welcome box: "Share your verify key with peers: `hsip status` shows it. They run `hsip trust add <label> <key>` to trust your messages." — closes the one item that had been sitting in "Remaining" since the Trust page/CLI shipped.
- `GET /v1/decisions` now reports which named connection recorded each decision (`agent_key_id`/`agent_name`/`agent_type`, via a `LEFT JOIN` on `api_keys` — a since-revoked key still lists its past decisions, just with all three `null`) and accepts `?agent_key_id=`/`?since_ms=`/`?until_ms=` filters, expressed as `($N IS NULL OR col = $N)` in one fixed query so both `sqlx::Any` backends stay covered without dynamic SQL. Closes a real UX gap raised directly by using the product: the "AI Decisions" dashboard page could show a plain-language decision feed but couldn't say *which* connected agent (or whether it was even an AI agent vs. a human) a decision belonged to, or narrow a lookup to one agent/time window — exactly what a compliance/audit reviewer needs first ("what did this agent do, in this window"), not a raw signature bundle. `DecisionsSimple.jsx`'s `DecisionRow` now shows a `🤖 <agent name> · AI agent/Human/Service` badge plus an absolute human-readable date (`formatDateTime`, alongside the existing relative "X minutes ago") on every row, the page gained agent/time-window filter dropdowns (backed by the existing `GET /v1/agents`) that re-query on change, and expanding a row now opens a plain-language "Who / What / When / Status" fact block *before* the technical prose/JSON — with `Status` auto-verified (not just "anchored") the instant the row opens, so a non-technical reviewer sees a real genuine/tampered verdict without a developer walking them through it. Covered by `test_decisions_list_reports_agent_name_and_filters_by_agent_and_time`. Also fixed a real, unrelated bug found in the same pass: `AIWatch.jsx` had `BASE_URL` hardcoded to `http://127.0.0.1:7777` (the wrong port for every deployment mode this project documents), silently breaking every copy-paste setup snippet (Siri Shortcut, Claude Desktop, capabilities URL) on that page — now `window.location.origin`, matching the pattern `DecisionsSimple.jsx` already used correctly.
- **First real, persistent public deployment is live**: `https://hsip-1phase-production-c45d.up.railway.app` (Railway, Postgres-backed, fixed `HSIP_ADMIN_KEY`, `HSIP_SANDBOX=true` for public 24h trial keys). Linked from the README, the landing page's hero CTA, and `docs/SANDBOX_QUICKSTART.md` (which previously pointed at a `demo.hsip.io` domain that was never actually real). See "Live Public Deployment" above.
- **Predicta integration is live, not just designed** — the client-side code was already fully built in Predicta's own repo; the missing piece was purely infrastructure (a real, Railway-reachable `HSIP_API_URL`/dedicated `HSIP_API_KEY`, both now set). Verified via a real human-authorized Buy showing up on HSIP's AI Decisions page within seconds, correctly attributed and cryptographically verified. See "Trading Bot Integration (Predicta) — Live" above.
- Messages page copy fixed to stop implying live delivery between HSIP instances — it never had any; every message is signed and stored locally, delivery is always manual. See "Federated Trust" region / `dashboard/src/pages/Messages.jsx`.
...
- **Encrypt `audit_entries.details` at rest** — the one field-encryption gap left open by the pass above. Needs a "hash the plaintext at write time (unchanged formula), encrypt only the persisted column" design so `audit_log::compute_entry_hash`/`GET /v1/audit/verify-proof`'s existing pure, DB-free, caller-supplied-plaintext contract isn't disturbed — genuinely more involved than `messages.content`/`credentials.claim` because of that entanglement, which is why it was deliberately scoped out rather than rushed. See THREAT_MODEL.md §4.30.
- **Database-compromise protection is now partial, not solved** — field encryption at rest (above) covers the two clearest cases of sensitive free text; `tenant_id`, `audit_entries.details`, and (until the item above lands) audit-log context remain plaintext, still linking every tenant's records together for anyone with raw DB access. Full "what does an attacker with the database file actually still learn" is worth a dedicated follow-up pass once the audit_entries gap closes.
- **Live-verify the `windows`-crate shortcut replacement on a real Windows machine** — §4.32/§4.33 replaced `mslnk` with direct `IShellLinkW`/`IPersistFile` COM calls, fixed a COM apartment-mode edge case, and made every shortcut failure show up in `install.log` instead of vanishing silently — all cross-checked against the real crate source and confirmed to cross-compile and link cleanly for `x86_64-pc-windows-gnu`, but this sandbox has no Windows host to actually run `maybe_self_install`/`create_shortcuts` and confirm a working `.lnk` appears on a real Desktop/Start Menu. Stays open until a real Windows user's build confirms it end-to-end — but that confirmation is now cheap to get: check `%LOCALAPPDATA%\HSIP\install.log` after running the installed copy, it will say `shortcut ok` or `shortcut FAILED: <reason>` for each of the two shortcut locations, no guessing required. See TESTING.md.
- **Checkpoint `GET /v1/audit/verify` instead of recomputing the whole chain every call** — `routes/audit.rs::verify_chain` fetches and re-verifies a tenant's *entire* audit history from genesis on every call, with no `LIMIT` and no stored "verified up to entry N" checkpoint. Cost grows unboundedly with audit log size for zero added value on already-verified history. Re-confirmed as real in §4.33's reasoning-QA pass but deliberately not fixed there — it touches the hash-chain verification path directly (the single most security-critical code in this system), and deserves the same dedicated, unhurried treatment §4.30 gave `audit_entries.details` rather than a same-session fix bolted on because it was already found. A checkpoint design needs to handle: what happens on a detected break (don't advance the checkpoint past it), and what the checkpoint's own storage/invalidation looks like — worth its own pass, not a quick patch.
